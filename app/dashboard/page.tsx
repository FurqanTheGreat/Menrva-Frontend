...
const Dashboard = () => {
  ...
  const sendPrompt = async (params: SendPromptParams): Promise<void> => {
    ...
    try {
      ...
      console.error("Error in sendPrompt:", error.response?.data || error.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className={`flex flex-row w-full h-full ${sourceSans3.className}`}>
      ...
      <div className={`w-full h-full flex flex-col items-center bg-[#27272aa8] ${!hid ? 'justify-center' : 'justify-end'}`}>
  {hid ? (
      ...
      {messages.map((msg, index) => (
        <Message
          ...
          message={msg.sender === "ai" ? (
            <ReactMarkdown remarkPlugins={[remarkGfm]} className="prose markdown">{msg.content}</ReactMarkdown>
          ) : (
            msg.content
          )}
          avatar={msg.sender === "user" ? "/Screenshot 2024-11-24 173312.png" : "/Colorful_Brain_Digital_World_Technology_Logo__3_-removebg-preview.png"}
  ...
  {!hid && <div className="w-1/2">
    <p className="text-5xl text-[#b6b6b6] text-center">Hi there, User</p>
    <br />
